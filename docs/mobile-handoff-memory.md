...
- Android: **v1.1.1 (versionCode 3)** собрано/подписано/выложено: https://splithub.ru/app/
  (старый APK забэкаплен на сервере `splithub.apk.bak-v110`). Коммит `a75cec2` на
  `codex/native-site-parity`, запушено в GitHub.
- **Финальное pre-prod QA (2026-07-04):** живой прогон на устройстве (TECNO BG6, ADB) —
  ревью кода + jest 91/91 + тест каталога/фильтров/корзины/заказа/отмены. Найдено и
  исправлено 5 багов:
  1. Смена фильтра/поиска не сбрасывала скролл каталога → `scrollToOffset(0)`.
  2. Протухший (>24ч) кэш каталога давал пустой экран при офлайне → TTL-гейт убран,
     показывается старый каталог с офлайн-баннером.
  3. Нет восстановления после потери сети → тихий авто-ретрай каталога каждые 30с;
     текст ошибки упоминает VPN.
  4. Заказы не обновлялись при повторном открытии вкладки → `useFocusEffect`;
     `repeat_order` теперь ловит сетевую ошибку.
  5. **Критично:** запуск приложения офлайн разлогинивал (любая сетевая ошибка при
     `profile` стирала токен) → токен чистится только при `SESSION_EXPIRED`;
     профиль довосстанавливается при возврате в foreground.
  - **⚠️ Открытие:** VPN-прокси на телефоне (Happ, немецкий туннель) блокирует
    splithub.ru целиком — не баг приложения, но частая причина «не грузится каталог»
    у клиентов с VPN. Смягчено фиксами 2–3 выше.
- iOS: TestFlight build 7, **Beta App Review = APPROVED** (2026-07-04) — публичная
  ссылка активна: https://testflight.apple.com/join/QxBBywEM. iOS-сборка ещё на
  1.1.0 (без сегодняшних Android-фиксов) — при желании пересобрать build 8.
...
- **Осталось:** чистка тест-аккаунтов в проде (`TestReg1634`, `test1634@example.com`, `ТЕСТ*`,
  `E2E*`, демо-аккаунт `+79000000099` после релиза в App Store); RuStore (2-й план,
  `docs/rustore-listing.md`); App Store публичная карточка (`docs/app-store-testflight-listing.md`).
